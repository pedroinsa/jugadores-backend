...
def get_all_posts_by_player(db: Session, player_id: int, skip: int = 0, limit: int = 20 ):
    ...
    return db.query(Post).filter(Post.player_id == player_id).order_by(Post.created_at.desc()).offset(skip).limit(limit).all()

#returns all posts that match the content search
def get_posts_by_content(db: Session, content: str, skip: int = 0, limit: int = 20):

    return db.query(Post).options(joinedload(Post.player)).filter(Post.content.ilike(f"%{content}%")).order_by(Post.created_at.desc()).offset(skip).limit(limit).all()
