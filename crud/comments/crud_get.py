...
def get_all_comments_from_post(db: Session, post_id: int, skip: int = 0, limit: int = 20):
   return db.query(Comment).options(joinedload(Comment.user)).filter(Comment.post_id == post_id).order_by(Comment.created_at.desc()).offset(skip).limit(limit).all()


#returns all comnents by content
def get_comments_by_content(db: Session, content: str, skip: int = 0, limit: int = 20):

    return db.query(Comment).options(joinedload(Comment.user)).filter(Comment.content.ilike(f"%{content}%")).order_by(Comment.created_at.desc()).offset(skip).limit(limit).all()
